...
- `obliqueCollectionName` can be used to set the oblique collection for the `ObliqueMap` and `ObliqueMultiView`. If not set, the oblique collection is taken from the mainMap, or if there is no Oblique mainMap just the first available oblique collection is used.
